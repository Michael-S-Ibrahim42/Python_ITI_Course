...
def extractFemaleNames(filename,letter):
  with open(filename, 'r') as file:
    lines = re.findall(r'(?:<.*?>)(\d+)(?:<.*?>)(?:<.*?>)(\w+)(?:<.*?>)(?:<.*?>)(\w+)(?:<.*?>)', file.read(), re.IGNORECASE)
  names = []
  for item in lines:
    if item[2].startswith(letter):
...
def extractDocAndPopularityDate(filename):
  output = ""
  with open(filename, 'r') as file:
    docDate = re.search(r'(?:<h1>.+</h1>)(.+)(?:</td>)', file.read(), re.IGNORECASE)
    file.seek(0)
    popDate = re.search(r'(?:<h3.*>)(.+)(?:</h3>)', file.read())
    output = docDate.group(1)+" and "+popDate.group(1)
  return output
